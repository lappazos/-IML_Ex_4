...
def load_images(path_to_data):
    images = []
    labels = []
    for filename in os.listdir(path_to_data + 'faces/FACES'):
        images.append(imread(os.path.join(path_to_data + 'faces/FACES', filename)))
        labels.append(1)
    for filename in os.listdir(path_to_data + 'faces/NFACES'):
        images.append(imread(os.path.join(path_to_data + 'faces/NFACES', filename)))
        labels.append(-1)
    return train_test_split(np.array(images, dtype=np.float64), np.array(labels, dtype=np.float64), test_size=0.33)
